...
// Function to seed a single template for testing
export async function seedSingleTemplate() {
  try {
    await dbConnect();
    ...
    if (existingCount === 0) {
      await Template.create(templateData[0]);
      console.log('Added a single template for testing');
    } else {
      console.log('Templates already exist, skipping seed');
    }
  } catch (error) {
    console.error('Error seeding single template:', error);
  }
}
