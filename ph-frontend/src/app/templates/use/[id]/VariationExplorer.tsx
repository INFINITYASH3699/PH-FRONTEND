...
export default function VariationExplorer({
  ...
  const [selectedTab, setSelectedTab] = useState<string>(activeSection ? 'section' : 'layout');
    ...
    <div className="p-4 bg-white rounded-lg border shadow-sm">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="w-full">
        ...
        <TabsContent value="section">
          {activeSection ? (
            <>
              <h3 className="text-sm font-medium mb-3 capitalize">{activeSection} Section Variants</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {sectionVariants.map((variant: any) => (
                    <Card
                      ...
                      onClick={() => onSectionVariantUpdate(activeSection, variant.id)}
                    >
              ...
              >
                <div className="p-3 flex justify-between items-start bg-gray-50 border-b">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{layout.structure?.gridSystem}</p>
                  </div>
                  {activeLayoutId === layout.id && (
                    <Badge className="bg-primary text-white">
                      <Check className="h-3 w-3 mr-1" />
        ...
        <TabsContent value="style">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Object.entries(stylePresets).map(([id, preset]: [string, any]) => (
              <Card
                key={id}
                className={`overflow-hidden cursor-pointer transition-all hover:border-primary ${
        ...
        <TabsContent value="animation">
          <div className="space-y-4">
            ...
            {animationsEnabled && (
              <>
                <h3 className="text-sm font-medium mt-4 mb-3">Animation Effects</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries(animations).map(([id, animation]: [string, any]) => (
                    <Card key={id} className="overflow-hidden">
                      ...
                      <CardContent className="p-3">
                        <div className="aspect-video bg-gray-100 rounded-md flex items-center justify-center">
                          ...
                          >
                            <span className="text-xs">{animation.type}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <div className="p-3 mt-3 bg-muted/30 rounded-md">
                  <p className="text-xs text-muted-foreground">
                    <strong>How animations are applied:</strong> Each section has a default animation
                    that triggers when the section comes into view. Header sections use fade-in,
                    about sections use slide-up, etc.
                  </p>
                </div>
              </>
            )}
